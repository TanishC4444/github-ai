...
`gh-ai-runner` is a Python orchestration package that turns a GitHub repository into an on-demand AI workload service. Version 0.2 adds non-blocking jobs, exact run correlation, cancellation, retries, structured token/resource telemetry, and securely stored keys for OpenAI-compatible providers. The original blocking `ai_call()` API remains compatible.
...
- The repository declares version `0.2.0` in both package metadata and the public module.
